...
## Datos estáticos de las 7 salas de la mazmorra.
...
## nueva — Main.gd construye la geometría por código en vez de instanciar.
##
...
##  - `entry`, cada `spawns[i]`, cada `props[i].pos` y cada `doors[x].pos`
##    tienen que caer DENTRO de la unión (o sobre su borde, en el caso de
##    las puertas).
##
## Campos de cada sala:
...
##                            (0 en room_boss: ahí se llama directo a
##                            Main._spawn_boss()).
...
##   doors: Dictionary      — lado ("east"/"west") -> {"to": id destino,
##                            "pos": Vector2 centro del vano SOBRE el borde}.
##                            1 entrada = puerta única, se abre sola.
##                            2 entradas = UpgradeUI en modo "door_pick".
...
static func _rooms() -> Dictionary:
	return {
		...
		"room_1": {
			...
			"enemy_types": ["grunt"],
			"doors": {
				"east": {"to": "room_2a", "pos": Vector2(400, 0)},
				"west": {"to": "room_2b", "pos": Vector2(-560, 0)},
			},
			"tint": null,
			"is_boss": false,
			"props": [
		...
		"room_2a": {
			...
			"doors": {"east": {"to": "room_3", "pos": Vector2(460, 135)}},
		...
		"room_2b": {
			...
			"doors": {"east": {"to": "room_3", "pos": Vector2(440, 0)}},
		...
		"room_3": {
			...
			"enemy_types": ["grunt", "runner", "spitter"],
			"doors": {
				"east": {"to": "room_4a", "pos": Vector2(620, 0)},
				"west": {"to": "room_4b", "pos": Vector2(-620, 0)},
			},
		...
		"room_4a": {
			...
			"waves": 3,
			"enemy_types": ["grunt", "runner", "spitter"],
			"doors": {"east": {"to": "room_boss", "pos": Vector2(80, -140)}},
		...
		"room_4b": {
			...
			"waves": 3,
			"enemy_types": ["grunt", "runner", "spitter"],
			"doors": {"east": {"to": "room_boss", "pos": Vector2(480, 0)}},
